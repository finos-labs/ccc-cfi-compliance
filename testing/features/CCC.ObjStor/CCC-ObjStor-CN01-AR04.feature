...
Feature: CCC.ObjStor.CN01.AR04
  ...
  Scenario: All unauthorized requests are blocked
    ...
    When I attempt policy check "object-storage-no-public-principals" for control "CCC.ObjStor.CN01" assessment requirement "AR04" for service "{ServiceType}" on resource "{ResourceName}" and provider "{Provider}"
    Then "{result}" is true
